...
export function ToolPage({ eyebrow, title, subtitle, toolInterface, features, steps, benefits }: ToolPageProps) {
  return (
    <>
      ...
      <Section className="bg-surface">
        ...
        <div className="grid sm:grid-cols-2 gap-4 max-w-3xl mx-auto">
          {features.map((f) => (
            <div key={f} className="flex items-start gap-3 rounded-xl border border-hairline bg-white p-4">
      ...
      <Section>
        ...
        <ol className="grid md:grid-cols-3 gap-6">
          {steps.map((s, i) => (
            <li key={s.title} className="rounded-2xl border border-hairline bg-white p-6">
      ...
      <Section className="bg-surface">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-4xl mx-auto">
          {benefits.map((b) => (
            <div key={b} className="rounded-xl bg-white border border-hairline p-5 text-ink">{b}</div>
          ))}
        </div>
