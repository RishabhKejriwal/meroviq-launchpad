    ...
    <>
      ...
      <Section>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {SVC.map((s) => (
            <div key={s.title} className="rounded-2xl border border-hairline p-7 bg-white shadow-soft">
        ...
        <div className="grid lg:grid-cols-2 gap-10 items-center">
          <div className="rounded-3xl border border-hairline bg-white p-8 shadow-soft">
            ...
            <div className="mt-6 flex items-end gap-2 h-48">
              {[30, 45, 38, 60, 52, 72, 65, 85, 78, 95].map((h, i) => (
                <div key={i} className="flex-1 rounded-t-md gradient-brand" style={{ height: `${h}%`, opacity: 0.35 + i * 0.06 }} />
              ))}
            </div>
            <div className="mt-6 grid grid-cols-3 gap-4 text-center">
              {[{l:"Impressions",v:"4.2M"},{l:"CTR",v:"6.4%"},{l:"ROAS",v:"4.8x"}].map(s=>(
                <div key={s.l}><div className="text-2xl font-bold text-brand">{s.v}</div><div className="text-xs text-ink-muted mt-1">{s.l}</div></div>
              ))}
            </div>
          </div>
          <div>
            ...
            <ul className="mt-6 space-y-3">
              {["Audience research and ICP mapping","Creative and content production","Paid + organic media buying","Conversion optimization & analytics","Weekly reporting and reviews"].map((t) => (
                <li key={t} className="flex items-start gap-3 text-ink"><TrendingUp className="h-5 w-5 text-accent2 mt-0.5" /> {t}</li>
              ))}
            </ul>
          </div>
        </div>
